...
In that order: `apps-and-tools` installs Homebrew and iTerm2, and `zsh` sets up
the prompt you'll use in them. Some steps ask for your password (the Homebrew
and AmneziaVPN installers, and the App Store helper). Installing a topic again
is safe: anything already in place is left alone, and anything replaced is
moved to `~/.dotfiles-backup/<timestamp>/` rather than deleted.

## 6. Open a new terminal tab

The zsh config only applies to shells started after the install. In iTerm2 also
set the **MesloLGS NF** font and the **Solarized Light** preset, so the prompt's
icons and colors are right — see
